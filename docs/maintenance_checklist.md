...
- [ ] LLM dataset size — `python -c` on `data/llm_generated.json` record count;
      missing scope should shrink as generation runs.
...
- [ ] New model → `.env` change (local only) → trial batch → judge
      French quality before any bulk run.
...
| symptom | cause → fix |
|---|---|
| `validate` reports overlap | lower-priority datasets stale vs CFDICT/human → run `scripts/cleanup.py`, review diff, commit |
...
| `validate` reports hanzi/pinyin | human entry disagrees with CC-CEDICT pairing/reading → fix `human.u8` |
